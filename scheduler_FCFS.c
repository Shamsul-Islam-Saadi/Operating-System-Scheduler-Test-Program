...
int main(){
	...
	if((p1.arrivalTime < p2.arrivalTime) && (p1.arrivalTime < p3.arrivalTime)){
		processOrder[0] = 1;
		if(p2.arrivalTime < p3.arrivalTime){
			processOrder[1] = 2;	
			processOrder[2] = 3;
		}
		else{
	...
	else if((p2.arrivalTime < p1.arrivalTime) &&(p2.arrivalTime < p3.arrivalTime)){
		processOrder[0] = 2;
		if(p1.arrivalTime < p3.arrivalTime){
			processOrder[1] = 1;	
			processOrder[2] = 3;
		}
		else{
	...
	printf("scheduler_FCFS: Starting work loop\n");			
	...
	printf("scheduler_FCFS: End of work loop\n");
	
	printf("FCFS Scheduler Ends!\n");
